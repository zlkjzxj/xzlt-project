...
;layui.define("jquery", function (e) {
    ...
    f.prototype.config = {
        ...
        if (y[0] && y.remove(), e.elemTemp = i(f), t.range ? (e.elemTemp.find("." + o).eq(0).data("value", t.value[0]), e.elemTemp.find("." + o).eq(1).data("value", t.value[1])) : e.elemTemp.find("." + o).data("value", t.value), h.html(e.elemTemp), "vertical" === t.type && e.elemTemp.height(t.height + "px"), t.showstep) {
            ...
            var b = "";
            for (var g = (t.max - t.min) / t.step, x = 1; x < g + 1; x++) {
                var T = 100 * x / g;
                T < 100 && (b += '<div class="layui-slider-step" style="' + ("vertical" === t.type ? "bottom" : "left") + ":" + T + '%"></div>')
            }
            if (JSON.stringify(e.config.tipvalues) !== "{}") {
                for (var g1 = (t.max - t.min) / t.step, x1 = 0; x1 < g1 + 1; x1++) {
                    var T1 = 100 * x1 / g1;
                    b += '<div class="layui-slider-tipvalues" style="' + ("vertical" === t.type ? "bottom" : "left") + ":" + T1 + '%;color:' + (t.value < T1 ? "#BFBFBF" : "#000") + '">' + t.tipvalues[T1] + '</div>';
                    // b += '<div class="layui-slider-step" style="width:30px;top:30px;' + ("vertical" === t.type ? "bottom" : "left") + ":" + T1 + '%;">' + t.tipvalues[T1] + '</div>';
                }
